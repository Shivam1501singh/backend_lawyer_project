...
## 34. Unified BNSS & BSA Legal-Content Module

This module provides a unified API surface for managing and querying **Bharatiya Nagarik Suraksha Sanhita (BNSS)** and **Bharatiya Sakshya Adhiniyam (BSA)** legal content while persisting data into **two separate, isolated database models/tables (`BNSS` and `BSA`)**.

### Database Architecture

The backend maintains two separate tables in PostgreSQL via Prisma:

```text
BNSS                                  BSA
--------------------------------      --------------------------------
id           String (UUID PK)         id           String (UUID PK)
sectionNo    String (Unique)          sectionNo    String (Unique)
heading      String                   heading      String
sectionText  Text                     sectionText  Text
explanation  Text (Optional/Nullable) explanation  Text (Optional/Nullable)
illustration Text (Optional/Nullable) illustration Text (Optional/Nullable)
...
| Endpoint | Method | Path | Access Level | Description |
| :--- | :--- | :--- | :--- | :--- |
| **Create Law** | `POST` | `/api/content-creator/laws` | `CONTENT_CREATOR` | Create entry in BNSS or BSA table |
| **Update Law** | `PATCH` | `/api/content-creator/laws/:id` | `CONTENT_CREATOR` | Update entry in BNSS or BSA table |
| **Delete Law** | `DELETE` | `/api/content-creator/laws/:id` | `CONTENT_CREATOR` | Delete entry in BNSS or BSA table |
| **List Laws** | `GET` | `/api/laws` | `PUBLIC` | Paginated list of BNSS or BSA sections |
| **Search Laws** | `GET` | `/api/laws/search` | `PUBLIC` | Case-insensitive multi-field search |
| **Get Law Section** | `GET` | `/api/laws/:id` | `PUBLIC` | Fetch single section by ID and lawType |

---

### Content Creator APIs

#### 1. Create Legal Entry (BNSS or BSA)
- **Endpoint:** `POST /api/content-creator/laws`
- **Authentication:** Bearer token / cookie with role `CONTENT_CREATOR`
- **Request Body (BNSS Example):**
  ```json
  {
    "lawType": "BNSS",
    "sectionNo": "1",
    "heading": "Short title, extent and commencement",
    "sectionText": "This Act may be called the Bharatiya Nagarik Suraksha Sanhita, 2023...",
    "explanation": "Preliminary applicability provisions.",
    "illustration": "Illustration of territorial jurisdiction."
  }
  ```
- **Request Body (BSA Example):**
  ```json
  {
    "lawType": "BSA",
    "sectionNo": "1",
    "heading": "Short title, application and commencement",
    "sectionText": "This Act may be called the Bharatiya Sakshya Adhiniyam, 2023...",
    "explanation": "Evidence rules in judicial proceedings.",
    "illustration": "Application to electronic records."
  }
  ```
- **Success Response (`201 Created`):**
  ...
  {
    "success": true,
    "message": "BNSS section created successfully",
    "data": {
      "id": "c7a8b410-6bf7-4eb4-b907-8e6d30f7b0e1",
      "lawType": "BNSS",
      "sectionNo": "1",
      "heading": "Short title, extent and commencement",
      "sectionText": "This Act may be called the Bharatiya Nagarik Suraksha Sanhita, 2023...",
...
#### 2. Update Legal Entry
- **Endpoint:** `PATCH /api/content-creator/laws/:id`
- **Authentication:** Bearer token / cookie with role `CONTENT_CREATOR`
- **Request Body:**
  ```json
  {
    ...
    "sectionText": "Updated section legal text...",
...
- **Success Response (`200 OK`):**
  ...
  {
    "success": true,
    "message": "BNSS section updated successfully",
    "data": {
      "id": "c7a8b410-6bf7-4eb4-b907-8e6d30f7b0e1",
      "lawType": "BNSS",
      "sectionNo": "1",
...
#### 3. Delete Legal Entry
- **Endpoint:** `DELETE /api/content-creator/laws/:id?lawType=BNSS` (or pass `lawType` in body)
- **Authentication:** Bearer token / cookie with role `CONTENT_CREATOR`
- **Success Response (`200 OK`):**
  ```json
  {
    "success": true,
    "message": "BNSS section deleted successfully"
  }
  ```
...
#### 1. List Legal Sections
- **Endpoint:** `GET /api/laws?lawType=BNSS&page=1&limit=10`
- **Query Parameters:**
  - `lawType` *(required)*: `BNSS` or `BSA`
  - `page` *(optional)*: Positive integer, default `1`
  - `limit` *(optional)*: Positive integer, default `10`, max `100`
- **Success Response (`200 OK`):**
  ```json
  {
    "success": true,
    "data": [
      {
        ...
        "sectionNo": "1",
        "heading": "Short title, extent and commencement",
        "sectionText": "This Act may be called the Bharatiya Nagarik Suraksha Sanhita...",
        "explanation": "Preliminary applicability provisions.",
        "illustration": null,
        "createdAt": "2026-09-22T08:00:00.000Z",
        "updatedAt": "2026-09-22T08:00:00.000Z"
...
- **Endpoint:** `GET /api/laws/:id?lawType=BNSS`
- **Query Parameters:**
  - `lawType` *(required)*: `BNSS` or `BSA`
- **Success Response (`200 OK`):**
  ```json
...
#### 3. Search Legal Sections
- **Endpoint:** `GET /api/laws/search?lawType=BNSS&q=arrest&page=1&limit=10`
- **Query Parameters:**
  - `lawType` *(required)*: `BNSS` or `BSA`
  - `q` *(required)*: Search term (e.g. `arrest`, `evidence`, `103`)
  - `page` *(optional)*: Positive integer, default `1`
  - `limit` *(optional)*: Positive integer, default `10`, max `100`
- **Search Logic:**
  - Case-insensitive database filtering (`mode: 'insensitive'`) matching `sectionNo`, `heading`, `sectionText`, `explanation`, and `illustration`.
  - Relevance ranking prioritizes exact section number matches, then title/heading matches, then explanations and content.
- **Success Response (`200 OK`):**
  ```json
  {
    "success": true,
    "data": [
      {
        ...
        "sectionText": "Any police officer may without an order from a Magistrate...",
        "explanation": "Arrest guidelines.",
        "illustration": null,
        "createdAt": "2026-09-22T08:00:00.000Z",
        "updatedAt": "2026-09-22T08:00:00.000Z"
      }
...
| :--- | :--- | :--- |
| **Invalid `lawType`** (e.g. `IPC`, `BNS`) | `400 Bad Request` | `{"success": false, "message": "lawType is required and must be either 'BNSS' or 'BSA'"}` |
| **Duplicate Section in Law** | `400 Bad Request` | `{"success": false, "message": "This BNSS section already exists"}` |
| **Missing Required Fields** | `400 Bad Request` | `{"success": false, "message": "sectionNo is required"}` |
| **Section Not Found** | `404 Not Found` | `{"success": false, "message": "BNSS section not found"}` |
| **Forbidden Role (User/Advocate)** | `403 Forbidden` | `{"success": false, "message": "Access forbidden. Content Creator role required."}` |
| **Unauthenticated Request** | `401 Unauthorized` | `{"success": false, "message": "Authentication required. Please login."}` |

---

### Postman Testing Guide

#### Test 1 — Create BNSS Section
1. Send `POST /api/content-creator/laws` with `CONTENT_CREATOR` Bearer token:
   ```json
   {
     "lawType": "BNSS",
     "sectionNo": "1",
     ...
     "sectionText": "This Act may be called the Bharatiya Nagarik Suraksha Sanhita, 2023...",
     "explanation": "BNSS preliminary explanation",
     "illustration": "BNSS illustration"
   }
   ```
2. **Verify (`201 Created`):**
   - Returns `"lawType": "BNSS"` and `"sectionNo": "1"`.

#### Test 2 — Create BSA Section (Same Section 1)
1. Send `POST /api/content-creator/laws` with `CONTENT_CREATOR` Bearer token:
   ```json
   {
     "lawType": "BSA",
     ...
     "sectionText": "This Act may be called the Bharatiya Sakshya Adhiniyam, 2023...",
     "explanation": "BSA evidence explanation",
     "illustration": "BSA illustration"
   }
   ```
2. **Verify (`201 Created`):**
   - Successfully created without collision with BNSS section 1.

#### Test 3 — Duplicate Rejection
1. Attempt to create BNSS section 1 again via `POST /api/content-creator/laws`.
2. **Verify (`400 Bad Request`):**
   - Message indicates `"This BNSS section already exists"`.

#### Test 4 — Public BNSS List & Isolation
1. Send `GET /api/laws?lawType=BNSS`.
2. **Verify (`200 OK`):**
   - Returns BNSS sections. No BSA sections appear in this response.

#### Test 5 — Public BSA List & Isolation
1. Send `GET /api/laws?lawType=BSA`.
2. **Verify (`200 OK`):**
   - Returns BSA sections. No BNSS sections appear in this response.

#### Test 6 — Public Search
1. Send `GET /api/laws/search?lawType=BNSS&q=commencement`.
2. **Verify (`200 OK`):**
   - Returns matching BNSS records.

#### Test 7 — Update Section
1. Send `PATCH /api/content-creator/laws/<id>` with:
   ```json
   {
     "lawType": "BNSS",
...
#### Test 8 — Delete Section
1. Send `DELETE /api/content-creator/laws/<id>?lawType=BNSS`.
2. **Verify (`200 OK`):**
   - Record is deleted.
   - Subsequent `GET /api/laws/<id>?lawType=BNSS` returns `404 Not Found`.

#### Test 9 — Role Security
1. Attempt `POST /api/content-creator/laws` as a Normal User -> **Verify `403 Forbidden`**.

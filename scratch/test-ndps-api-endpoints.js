...
async function runApiTests() {
  console.log('Testing Bearer Act endpoints for THE NARCOTIC DRUGS AND PSYCHOTROPIC SUBSTANCES ACT, 1985 at', BASE_URL);
  ...
  try {
    ...
    const ndpsAct = acts.find(a => a.heading === 'THE NARCOTIC DRUGS AND PSYCHOTROPIC SUBSTANCES ACT, 1985');
    assert(!!ndpsAct, 'NDPS Act is listed in Criminal category');
    assert(ndpsAct.year === 1985, 'NDPS Act year is 1985');
    ...
    assert(actsRes.data.data.some(a => a.heading === 'THE NARCOTIC DRUGS AND PSYCHOTROPIC SUBSTANCES ACT, 1985'), 'NDPS Act found in acts endpoint');

    // 4. GET /api/acts/:id
    console.log('\n[4] Testing GET /api/acts/:id...');
    const singleActRes = await axios.get(`${BASE_URL}/api/acts/${ndpsAct.id}`);
    assert(singleActRes.status === 200, 'GET /api/acts/:id returns 200');
    assert(singleActRes.data.data.heading === 'THE NARCOTIC DRUGS AND PSYCHOTROPIC SUBSTANCES ACT, 1985', 'Single act heading matches');
    assert(singleActRes.data.data.sections.length === 129, `Single act includes 129 sections (found: ${singleActRes.data.data.sections.length})`);
    ...
    const sectionsPage1 = await axios.get(`${BASE_URL}/api/acts/${ndpsAct.id}/sections?page=1&limit=10`);
    ...
    assert(sectionsPage1.data.pagination.total === 129, 'Total count is 129');
    assert(sectionsPage1.data.pagination.totalPages === 13, 'Total pages is 13');
    ...
    const searchQueries = ['Narcotic', 'Psychotropic', 'Substances', 'Cannabis', 'Opium', 'Poppy', 'Forfeiture', 'Special Court'];

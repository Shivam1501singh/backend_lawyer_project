...
async function runApiTests() {
  console.log('Testing Bearer Act endpoints for THE SEXUAL HARASSMENT OF WOMEN AT WORKPLACE ACT, 2013 at', BASE_URL);
  ...
  try {
    ...
    const poshAct = acts.find(a => a.heading === 'THE SEXUAL HARASSMENT OF WOMEN AT WORKPLACE (PREVENTION, PROHIBITION AND REDRESSAL) ACT, 2013');
    assert(!!poshAct, 'POSH Act is listed in Criminal category');
    assert(poshAct.year === 2013, 'POSH Act year is 2013');
    ...
    assert(actsRes.data.data.some(a => a.heading === 'THE SEXUAL HARASSMENT OF WOMEN AT WORKPLACE (PREVENTION, PROHIBITION AND REDRESSAL) ACT, 2013'), 'POSH Act found in acts endpoint');

    // 4. GET /api/acts/:id
    console.log('\n[4] Testing GET /api/acts/:id...');
    const singleActRes = await axios.get(`${BASE_URL}/api/acts/${poshAct.id}`);
    assert(singleActRes.status === 200, 'GET /api/acts/:id returns 200');
    assert(singleActRes.data.data.heading === 'THE SEXUAL HARASSMENT OF WOMEN AT WORKPLACE (PREVENTION, PROHIBITION AND REDRESSAL) ACT, 2013', 'Single act heading matches');
    assert(singleActRes.data.data.sections.length === 30, `Single act includes 30 sections (found: ${singleActRes.data.data.sections.length})`);
    ...
    const sectionsPage1 = await axios.get(`${BASE_URL}/api/acts/${poshAct.id}/sections?page=1&limit=10`);
    ...
    assert(sectionsPage1.data.pagination.total === 30, 'Total count is 30');
    ...
    const searchQueries = ['Sexual', 'Harassment', 'Women', 'Workplace', 'Prevention', 'Prohibition', 'Redressal', 'Complaints Committee', 'Conciliation'];
    for (const q of searchQueries) {
      ...
      assert(searchRes.data.data.length > 0, `Search for "${q}" returned ${searchRes.data.data.length} results`);
    }

  } catch (err) {
    console.error('Error during API testing:', err.response?.data || err.message);
    failed++;
  } finally {
